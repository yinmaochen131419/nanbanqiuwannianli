...
object FourPillarsEngine {
    ...
    fun calculate(
        ...
        val lunarMonthName = if (lunarResult.isLeapMonth) {
            "闰${CalendarConstants.LUNAR_MONTH_NAMES.getOrElse(lunarResult.month - 1) { "${lunarResult.month}月" }}"
        } else {
            CalendarConstants.LUNAR_MONTH_NAMES.getOrElse(lunarResult.month - 1) { "${lunarResult.month}月" }
        }
